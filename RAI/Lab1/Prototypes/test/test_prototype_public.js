...
import { Pojazd } from '../src/hoisting_prototype_public.js';
...
describe('Pojazd', function()
{
    ...
    it('should have r/w access to fields', function(){
        pojazd.speed = 0;
        expect(pojazd.getSpeed()).to.eql(0);
        pojazd.setSpeed(50);
        expect(pojazd.speed).equal(50);
        
        // this feels over-exstensive
        expect(pojazd.top_speed).equal(100);
        pojazd.top_speed = 200;
        pojazd.setSpeed(240);
        expect(pojazd.getSpeed()).equal(200);
    });

    it('should own fields', function(){
        expect(pojazd).to.haveOwnProperty('speed');
        expect(pojazd).to.haveOwnProperty('id');
        expect(pojazd).to.haveOwnProperty('top_speed');
    });
    ...
    it('should react to prototype change', function(){
        Pojazd.prototype.testFunction = function(){return 'test'};
